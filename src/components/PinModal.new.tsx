...
const PinModal = ({ mode, onClose, onSuccess, availableQuestions }: PinModalProps): React.ReactElement => {
  ...
  const handleRecovery = async () => {
    ...
    if (!isInRecoveryMode) {
      ...
      try {
        ...
        const sent = await sendRecoveryEmail(code);
  ...
  const handleSubmitPinManagement = async () => {
    setError('');
    setMessage('');
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (!isInRecoveryMode && window.confirm(
                            'Initiate secure account recovery? You will need internet access to receive a recovery code via email.'
                          )) {
                            handleRecovery();
                          }
                        }}
              ...
              {mode === 'disable' ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {mode === 'setup' && (
                      <p className="text-xs text-muted-foreground mt-1">
                        This email will be used to send a recovery code if you forget your PIN.
                        Ensure this is a valid email address you have access to.
                      </p>
                    )}
                  </div>
                </div>
